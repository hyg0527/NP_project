...
public class Server {
    ...
    private JButton btnRecordScore, btnRollDice, resetButton;
    ...
    class UserService extends Thread {
        ...
        public UserService(Socket client_socket, int count) {
            // 매개변수로 넘어온 자료 저장
        ...
        public void turnOver() {
        	...
        	if (!isTurn) {
        		user = user_vc.get(0);
        		user.WriteAll("player1 차례입니다.\n");
        		AppendText("player1 차례입니다.");
        	}
        	else if (isTurn) {
        		user = user_vc.get(1);
        		user.WriteAll("player2 차례입니다.\n");
        		AppendText("player2 차례입니다.");
        	}
        	user.WriteOne("//player");
        }
            ...
            while (true) {
                try {
                    ...
                    if (msg.equals("/startGame")) {
                    	WriteAll("게임을 시작합니다!!\n");
                    	WriteAll("//startGame");
                    	AppendText("유저가 모두 모여 게임을 시작합니다.");
                    	turnOver();
                    }
